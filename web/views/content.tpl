...
<html>
	...
	<body>
		...
		<div class="container">
			<div class="clearfix" style="font-size: 18px;"><p style="font-size: 20px; "><strong>application:&nbsp;</strong>{{viewmodel['app_name']}}</p><strong>hosts:</strong>
%for ip in viewmodel["ips"]:
			<a href="/content?ip={{ip['audit_ip_encode']}}&app={{ip['audit_app_encode']}}">{{ip['audit_ip']}}</a>;
%end
				<p>&nbsp;</p>
			...
			<div id="treeMenu">
				<div width="100%"  class="tr_tit" id="tr_tit">
					<ul class="clearfix">
						<li class="h-32 col_2">&nbsp;=^_^=&nbsp;application:{{viewmodel['app_name']}}&nbsp;^_^&nbsp;host_ip:{{viewmodel['ip_address']}}&nbsp;=^_^=</li>
						<li class="h-32 col_3">attachments</li>
						<li class="h-32 col_4">S/F</li>
...
%for item in viewmodel["rows"]:
				<ul class="tree_content clearfix ul_bg">
					<li data-id="{{item['audit_id']}}" class="clearfix" node-index="0">
						...
						<div class="col_4 col_item" title="{{item['async']}}">{{item['status']}}:{{item['async']}}</div>
...
%end
				...
				<div class="pager">
					<div class="scott2">
						...
						<a href="?page={{viewmodel['prepage']}}&ip={{viewmodel['ip_en']}}&app={{viewmodel['app_en']}}">上一页</a>

						<span class="current">{{viewmodel['curpage']}}</span>

						<a href="?page={{viewmodel['afterpage']}}&ip={{viewmodel['ip_en']}}&app={{viewmodel['app_en']}}">下一页</a>
